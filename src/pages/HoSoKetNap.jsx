      ...
      try {
        ...
        const parsed = jsonData.map((row, index) => {
          ...
          const mappedRow = {
            ...
            soqd: row["Số quyết định"] || row["SỐ QĐ"] || row["Số QĐ"] || row["Số quyết định kết nạp"] || row["SỐ QUYẾT ĐỊNH KẾT NẠP"] || row["Số QĐ kết nạp"] || row["Số QĐ KN"] || "",
            ngaykiqd: parseExcelDate(row["Ngày ký quyết định"] || row["Ngày kí"] || row["NGÀY KÍ"] || row["NGÀY KÍ "] || row["Ngày ký QĐ"] || row["NGÀY KÝ"] || row["NGÀY KÝ QUYẾT ĐỊNH"] || row["Ngày ký quyết định kết nạp"] || row["Ngày kí quyết định kết nạp"] || row["NGÀY KÝ QUYẾT ĐỊNH KẾT NẠP"] || row["Ngày ký QĐ kết nạp"] || row["Ngày kí QĐ kết nạp"]),

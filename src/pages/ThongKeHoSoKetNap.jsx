...
const ThongKeHoSoKetNap = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const q = collection(dbMain, "ho_so_ket_nap");
      const querySnapshot = await getDocs(q);
      const docs = querySnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      setData(docs);
    } catch (error) {
      console.error("Lỗi tải dữ liệu thống kê:", error);
      message.error("Lỗi tải dữ liệu thống kê hồ sơ kết nạp");
    } finally {
      setLoading(false);
    }
  };
  ...
  const stats = useMemo(() => {
    ...
    // 1. Tổng số hồ sơ đang làm (Steps 1-7)
    const inProgress = filteredData.filter(item => {
      const step = Number(item.trangthai || 1);
      return step >= 1 && step <= 7;
    }).length;
    
    // 2. Tổng số hồ sơ đã có quyết định kết nạp (Step 8)
    const admitted = filteredData.filter(item => {
      const step = Number(item.trangthai || 1);
      return step === 8;
    }).length;
    
    // 3. Tổng số hồ sơ Đã nộp lên Đảng ủy ĐHĐN (Strictly Step 7: Hồ sơ gửi ĐHĐN)
    ...
    // Yearly/Monthly trends (Specifically for admitted profiles)
    ...
    filteredData.forEach(item => {
      const step = Number(item.trangthai || 1);
      const isAdmitted = step === 8;
      const isInProgress = step >= 1 && step <= 7;
      ...
      if (isAdmitted) facultyMap[f].admitted++;
      if (isInProgress) facultyMap[f].inProgress++;

      // Gender Grouping
      const g = item.gioitinh || 'Nam';
      if (genderMap[g]) {
        if (isAdmitted) genderMap[g].admitted++;
        if (isInProgress) genderMap[g].inProgress++;
      }
      ...
      if (isAdmitted) cohortMap[c].admitted++;
      if (isInProgress) cohortMap[c].inProgress++;
      ...
      if (isAdmitted) facultyCohortMap[fcKey].admitted++;
      if (isInProgress) facultyCohortMap[fcKey].inProgress++;

      // Decision Date trends (Admitted Step 8)
      if (isAdmitted) {
        const dateStr = item.ngayvaodang || item.ngaynhanhoso || item.created_at;
        if (dateStr) {
    ...
    for (let i = 0; i < 4; i++) {
      activeYears.add(String(currentYear - i));
    }
  ...
  const trendTableColumns = [
    ...
    { 
      title: 'Số lượng đã kết nạp', 
      dataIndex: 'value', 
      key: 'value', 
      render: val => <strong style={{ fontSize: '14px' }}>{val} hồ sơ</strong> 
    }
  ];
  ...
  const facultyTableColumns = [
    ...
    { 
      ...
      fixed: 'right',
      width: 110,
      render: val => <strong>{val} hồ sơ</strong> 
    }
  ];

  const cohortTableColumns = [
    { title: 'Khóa học', dataIndex: 'name', key: 'name', render: text => <strong>{text}</strong> },
    ...(stats.inProgress > 0 ? [{ title: 'Đang làm', dataIndex: 'Đang làm', key: 'inProgress', render: val => <span style={{ color: '#fa8c16', fontWeight: 'bold' }}>{val} hồ sơ</span> }] : []),
    { title: 'Đã kết nạp', dataIndex: 'Đã kết nạp', key: 'admitted', render: val => <span style={{ color: '#c62828', fontWeight: 'bold' }}>{val} hồ sơ</span> },
  ...
  return (
    <div className="dashboard-wrapper">
      ...
      <div className="dashboard-header-row">
        <div>
          <h1 className="dashboard-header-title">THỐNG KÊ HỒ SƠ KẾT NẠP ĐẢNG</h1>
          <Text type="secondary" style={{ fontSize: 13 }}>
            Hệ thống báo cáo số liệu và phân tích cơ cấu hồ sơ kết nạp Đảng viên - Chi bộ Sinh viên
          </Text>
        </div>
            ...
            <div>
              <div className="kpi-value-compact">
                ...
                <span style={{ fontSize: '13px', fontWeight: 'normal', color: '#8c8c8c', marginLeft: '12px' }}>
                  (Trong đó đã nộp ĐHĐN: <strong style={{ color: '#1890ff' }}>{stats.submittedDhdn}</strong>)
                </span>
              </div>
              <div className="kpi-label-compact">Tổng số hồ sơ đang làm</div>
            </div>
          </div>
        </div>

        {/* KPI 2: Đã có quyết định kết nạp */}
        <div className="kpi-card-compact" style={{ borderLeft: '4px solid #c62828' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '14px' }}>
            ...
            <div>
              ...
              <div className="kpi-label-compact">Tổng số hồ sơ đã kết nạp</div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div>
            ...
            <Row gutter={[16, 16]} style={{ marginBottom: 16 }}>
                  ...
                  <div style={{ height: 260 }}>
                    <ResponsiveContainer width="100%" height="100%">
                      <BarChart data={stats.cohortData} margin={{ top: 10, right: 10, left: -25, bottom: 0 }}>
                        ...
                        <Bar dataKey="Đã kết nạp" name="Đã kết nạp" fill="#c62828" radius={[4, 4, 0, 0]} barSize={16} />
                      </BarChart>
                    </ResponsiveContainer>
                  </div>
                </div>
              ...
              <Col xs={24} lg={12}>
                <div className="chart-card">
                  <div className="chart-card-header">
                    <span className="chart-card-title">CƠ CẤU GIỚI TÍNH HỒ SƠ KẾT NẠP</span>
                  </div>
                  <div style={{ height: 260, display: 'flex', justifyContent: 'space-around', alignItems: 'center' }}>
                    {/* Left Pie: Đang làm */}
                    {stats.inProgress > 0 && (
                      <div style={{ textAlign: 'center', width: '45%' }}>
                        <div style={{ fontSize: '12px', fontWeight: 700, color: '#fa8c16', marginBottom: 8 }}>Đang làm</div>
                    ...
                    {/* Right Pie: Đã kết nạp */}
                    <div style={{ textAlign: 'center', width: stats.inProgress > 0 ? '45%' : '100%' }}>
                      <div style={{ fontSize: '12px', fontWeight: 700, color: '#c62828', marginBottom: 8 }}>Đã kết nạp</div>
                      <div style={{ height: 150 }}>
                        <ResponsiveContainer width="100%" height="100%">
                          <PieChart>
                            <Pie
                      ...
                      <div style={{ fontSize: '11px', fontWeight: 700, marginTop: 4 }}>
                        <span style={{ color: '#c62828' }}>Nam: {stats.genderData[0]["Đã kết nạp"]}</span> | <span style={{ color: '#d4af37' }}>Nữ: {stats.genderData[1]["Đã kết nạp"]}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="chart-card">
                  <div className="chart-card-header">
                    <span className="chart-card-title">
                      BIẾN ĐỘNG HỒ SƠ CÓ QUYẾT ĐỊNH KẾT NẠP
                    </span>
                    <Radio.Group 
                      value={timeTrendType} 
...
export default ThongKeHoSoKetNap;
